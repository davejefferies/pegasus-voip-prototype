...
class AGI
{
    ...
    */
    ...
    {
      $this->verbose("GET DATA $filename $timeout $max_digits");
    ...
    */
    ...
    function notice($message, $level=1)
    {
        foreach(explode("\n", str_replace("\r\n", "\n", print_r($message, true))) as $msg)
        {
            @syslog(LOG_WARNING, $msg);
    ...
    //            $this->conlog("evaluate error on multiline read for $command");
        ...
        if($ret['result'] < 0)
          $this->conlog("$command returned {$ret['result']}");
				//if (substr($command, 0, 7) != 'VERBOSE') {
				//	$this->verbose($command . ', ' . json_encode($ret));
				//}
        return $ret;
    }
    /**
